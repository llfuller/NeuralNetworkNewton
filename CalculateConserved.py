...
def energy(m1, m2, v1, v2):
    # total kinetic energy of two masses
...
def energy_one(m1, v1): # For file Net_Single_Kinematic_Timestep_Plus_Conservation.py
    # total kinetic energy of one mass moving in one dimension
    v1_norm_squared = sp.array([sp.power(v1[trial],2) for trial in range(len(v1))])
    E1 = sp.multiply(0.5,  sp.multiply(m1,v1_norm_squared))
    return sp.real(E1)
def x_momentum(m1, m2, v1, v2): # Total x momentum for two particles
...
def y_momentum(m1, m2, v1, v2): # Total y momentum for two particles
    if (v1.shape)[-1] == 2:  # if second index of v1 is 2 (2 rows implies one for x, and one for y)
    ...
    return sp.real(p1 + p2)
def momentum_one(m1, v1): # For file Net_Single_Kinematic_Timestep_Plus_Conservation.py
    p1 = sp.multiply(m1, v1)
    return sp.real(p1)
